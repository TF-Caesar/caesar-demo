...
/** Read a whole NDJSON response body into parsed lines (test-sized bodies only). */
...
describe('POST /api/find/stream', () => {
  ...
  it('rejects an empty or unparseable query with 400 before any Caesar work', async () => {
    for (const body of [JSON.stringify({ query: '   ' }), 'not json']) {
